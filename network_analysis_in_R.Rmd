...
- Leading eigenvector (`cluster_leading_eigen`)
    + Uses the leading non-negative eigenvector of the modularity matrix, analagous to spectral partitioning discussed earlier (@newman_community06).

There are many other ways to measure community structure. For example, blockmodels look for *structurally equivalent* communities, where nodes in a block relate similarity to other blocks. A full review is beyond our scope, but we mention another flavor of metric implementable in `igraph` for comparision.
...
- Infomap (`cluster_infomap`)
    + Minimize the expected ``description length'' of a random walker.
...
- Reciprocity: 
    ...
    + $\frac{\sum_{i, j} (B_{ij}B'_{ij})}{\sum_{ij}(B_{ij})}$ , where $B_{ij}$ is the binary representation of $A_{ij}$. 
    + For valued networks you can use a simple correlation measure instead.
...
- Assortativity:
    ...
    + $\sum_{ij}(e(i,j)-q_{out}(i)q_{in}(j)) / \sigma(q_{in}) * \sigma(q_{out})$, where  
      $q_{out}(i) = \sum_j e(i,j)$, q_{in}(i)=\sum_j e(j,i)$, just the total weight out and in.
    + **by characteristic**: Since we don't have nodal characteristics we'll use communities to illustrate
...
assortativity_nominal(Cgraph.bin, types = cl.membership.louvain, directed = T)
```
...
- Clustering coefficient or transitivity:
    ...
    + `transitivity` method in `igraph` assumed an undirected binary network.
...
- Using the journal data, apply a measure for the following not discussed
    ...
    + Clustering, and compare the modularity. 
    + Another graph measurement or statistic.
...
`latentnet` (version 2.9.0)
Depends:	`R` (≥ 2.8.0), `statnet.common` (≥ 3.1.0), `network`, `ergm` (≥ 3.9.0)
Imports:	`sna`, `mvtnorm`, `abind`, `coda` (≥ 0.17.1), `tools`, `MASS`

A more specialized package whose authors overlap with `statnet`, but the model has roots in dissimilarity measures, but in the context of a model.
...
`latentnet` allows you to find the maximum likelihood estimate using fast method by setting `tofit = "mle"`, but you don't get complete information about the model.

## B.) Example Model Fit: Network of Journal Citations
- `euclidean(d=2)` argument indicates that the latent positions are two-dimensional.
- `response` argument conveys a valued network 
- `rsender` and `rreceiver` add terms for each node that describe in-strength and out-strength of ties.
...
A better but slower estimation method is to use MCMC, which gives you information about uncertainty in the estimated positions.
...
latent.srp2 = ergmm(Cnet ~ euclidean(d = 2) + rsender + rreceiver,
                    ...
                    control = ergmm.control(burnin = 10000, interval = 100, sample.size = 5000))
t2 = Sys.time()
t2 - t1 #takes about a minute

```

- Always check `mcmc.diagnostics` of your model. 

- We can plot the resulting positions and their uncertainty 

## C.) Inspect the Output
```{r latent_2_plot, eval = T, echo = T, cache = T, warning = F, message = F, result = "hide", fig.keep = "last"}
...
```{r journals_latent_clustering}
{plot(latent.srp2$mkl$Z, col = 1 + cl.membership.louvain, cex = vc2, pch = 16,
     bty="n", yaxt="n", xaxt="n", xlab = "", ylab = "", ylim = c(-2,3),
     main = "Visualization of community detection using latent positions")
text(latent.srp2$mkl$Z, labels = rownames(Cmatrix), cex = .5, pos = 3, offset = .3)}
```

## D.) Dynamic Visualization

Finally, we can use an interactive plot to better-visualize the output.
...
We can use the drop-down to compare the various similarity/distance metrics and use the color and width to highlight the strength of relationships.

These are the other metrics in the drop-down. We let the diagonal of the adjacency matrix equal the number of appearances of each justice:
...
As expected, the Kulczynski metric can highlight relationships outside the nodes that occur most often. The Haberman metric, and with it the P(Z) can as well, reflecting their standardization.
...
- "Network" by M.E.J. Newman. (In the first edition numbering) Chapter 7 "Measures and metrics" and Chapter 11 "Matrix algorithms and graph partitioning" overlaps most with the underlying material of this tutorial (but the book doesn't get into code) (@newman10). 
